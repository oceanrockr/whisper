    ...
    def transcribe_and_type(self):
        ...
        try:
            ...
            if len(audio) < self.sample_rate * 0.3:  # Less than 0.3 seconds
                self.status_var.set("⚠️ Audio too short - try speaking longer")
                self.footer_var.set("Ready to record")
                return
            ...
            if text:
                ...
                self.status_var.set(f"⌨️ Typing: {text[:50]}...")
                ...
                self.status_var.set(f"✓ Typed: {text[:50]}...")
                self.footer_var.set(f"Success! Last typed: {len(text)} characters")
            else:
            ...
            self.footer_var.set("Error occurred - check console")
