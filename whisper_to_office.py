...
def transcribe_for_powerpoint(audio_file, model_name="base", output_file=None):
    ...
    try:
        ...
        with open(safe_path, 'w', encoding='utf-8') as f:
            ...
            f.write("=" * 70 + "\n\n")
            f.write(f"Source: {Path(audio_file).name}\n")  # Don't expose full path
            f.write(f"Language: {result['language']}\n\n")
        ...
        print(f"\n✓ PowerPoint notes saved to: {safe_path}")
        ...
        logger.error(f"Error writing PowerPoint notes: {e}", exc_info=True)
        print(f"\n✗ Error: Failed to save PowerPoint notes")
        return None
...
def transcribe_meeting_minutes(audio_file, model_name="base", output_file=None):
    ...
    try:
        ...
        with open(safe_path, 'w', encoding='utf-8') as f:
            ...
            f.write(f"Recording: {Path(audio_file).name}\n\n")  # Don't expose full path
        ...
        print(f"\n✓ Meeting minutes saved to: {safe_path}")
        ...
        logger.error(f"Error writing meeting minutes: {e}", exc_info=True)
